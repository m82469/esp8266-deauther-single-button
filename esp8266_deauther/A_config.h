...
// Anyx ESP8266 V3 + 0.96" OLED (CH340) — reverse-engineered from a known-working
// sketch: SSD1306 driven over bit-banged I2C on GPIO12/GPIO14, one physical
// button on GPIO0 (the FLASH button). See setupButtons() in DisplayUI.cpp for
// how a single button is made to do both "next item" and "select".
#elif defined(ANYX_ESP8266_V3_OLED)
...
// ===== DISPLAY ===== //
  ...
  #define I2C_SCL 12 // OLED SCL

// ===== BUTTONS ===== //
  #define BUTTON_DOWN 0 // the only physical button (FLASH/GPIO0): tap = next item, hold = select
